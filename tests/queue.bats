...
@test "queue file: creates an issue and embeds the W1 envelope + record type" {
  run scripts/queue.sh file --persona "Ben" --tier "finances Team · Developer" \
      --type ASSESSMENT --title "clock skew 401" --body "details"
  [ "$status" -eq 0 ]
  echo "$output" | grep -qF 'issues/42'
  grep -q "issue create" "$PL_GH_LOG"
  grep -q -- "--body" "$PL_GH_LOG" && grep -q "ASSESSMENT" "$PL_GH_LOG"
...
@test "queue file: envelope embeds the persona avatar img" {
  run scripts/queue.sh file --persona "Ben" --tier "finances Team · Developer" --type ASSESSMENT --title t --body b
  [ "$status" -eq 0 ]
  grep -q "avatars/ben/ben-64.png" "$PL_GH_LOG"
}

@test "queue comment: appends an enveloped comment to an issue" {
  run scripts/queue.sh comment 42 --persona Ben --tier "finances Team · Developer" --type DELIVERED --body "fixed"
  [ "$status" -eq 0 ]; grep -q "issue comment 42" "$PL_GH_LOG"
  grep -q "DELIVERED" "$PL_GH_LOG"
}
...
@test "queue file --repo: targets the named repo" {
  run scripts/queue.sh file --repo o/r --persona Ben --tier "t · Developer" --type ASSESSMENT --title x --body y
...
@test "queue file without --repo: no stray --repo flag" {
  run scripts/queue.sh file --persona Ben --tier "t · Developer" --type ASSESSMENT --title x --body y
  [ "$status" -eq 0 ]
